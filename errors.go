...
func (err *Error) Error() string {
	...
	for _, prefix := range err.prefix {
		builder.WriteByte('[')
		builder.WriteString(ToString(prefix))
		builder.WriteString("] ")
	}
...
func (err *Error) AtDebug() *Error {
	return err.atSeverity(Severity_Debug)
}

// AtInfo sets the severity to info.
func (err *Error) AtInfo() *Error {
	return err.atSeverity(Severity_Info)
}

// AtWarning sets the severity to warning.
func (err *Error) AtWarning() *Error {
	return err.atSeverity(Severity_Warning)
}

// AtError sets the severity to error.
func (err *Error) AtError() *Error {
	return err.atSeverity(Severity_Error)
}
